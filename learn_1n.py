...
ds/dt = 0/ms : 1
"""
R = NeuronGroup(1, model = response_model, method = 'euler')
R.reward = -1*ms
R.response = coincidence_window
SR = Synapses(N, R, pre = 's += 1', connect = 'i == stimulus_size')
...
def response_model():
	if int(R.t/ms) % 10000 == 0:
		R.s = 0
	if int(R.t/ms) % 10000 == 100:
		response = R.s[0]
		responses.append(response)
		if response == 1:
			R.reward = R.t + reward_delay / (response + 1)
